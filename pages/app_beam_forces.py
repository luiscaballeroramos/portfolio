...
P = 1   # Valor de la carga (N), para normalización

# Slider para posición de la carga puntual
a = st.slider("📍 Posición de la carga puntual (m)", min_value=0.0, max_value=L, value=L/2, step=0.1)
...
st.latex(r"V(x) = \begin{cases} R_A = P \cdot \frac{L - a}{L} & \text{si } x < a \\
R_A - P & \text{si } x \geq a \end{cases}")
st.latex(r"M(x) = \begin{cases} R_A \cdot x & \text{si } x < a \\
R_A \cdot x - P \cdot (x - a) & \text{si } x \geq a \end{cases}")
